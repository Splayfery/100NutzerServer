    ...
    public void onSlashCommandInteraction (SlashCommandInteractionEvent event) {
        ...
        if (event.getName().equals("unlock")) {
            ...
            if (event.getMember().hasPermission(event.getGuildChannel(), Permission.MANAGE_PERMISSIONS)) {
                ...
                List<Permission> allowd = List.of(Permission.VIEW_CHANNEL, Permission.MESSAGE_MANAGE, Permission.MANAGE_CHANNEL, Permission.MESSAGE_SEND);
                ...
                switch (event.getChannelType()) {
                    ...
                    case TEXT:
                        ...
                        event.getGuild().getTextChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(everyone).grant(allowd).queue();

                        break;

                    case VOICE:

                        event.getGuild().getVoiceChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(everyone).grant(allowd).queue();

                        break;

                    case CATEGORY:

                        event.getGuild().getCategoryById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(everyone).grant(allowd).queue();

                        break;

                    case STAGE:

                        event.getGuild().getStageChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(everyone).grant(allowd).queue();

                        break;

                    case NEWS:

                        event.getGuild().getNewsChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(everyone).grant(allowd).queue();
                ...
                reply.setTitle("✅ **KANAL ERFOLGREICH ENTSPERRT!**");
                reply.setDescription("> Du hast den Kanal nun erfolgreich für alle Nutzer entsperrt!");
