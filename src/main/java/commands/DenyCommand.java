    ...
    public void onSlashCommandInteraction (SlashCommandInteractionEvent event) {
        ...
        if (event.getName().equals("deny")) {
            ...
            if (event.getMember().hasPermission(event.getGuildChannel(), Permission.MANAGE_PERMISSIONS)) {
                ...
                List<Permission> deny = List.of(Permission.MESSAGE_MANAGE, Permission.MANAGE_CHANNEL, Permission.MESSAGE_SEND);
                ...
                Member member = event.getOptionsByName("nutzer").get(0).getAsMember();
                    ...
                    switch (event.getChannelType()) {
                        ...
                        case TEXT:
                            ...
                            event.getGuild().getTextChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(member).deny(deny).queue();

                            break;

                        case VOICE:

                            event.getGuild().getVoiceChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(member).deny(deny).queue();

                            break;

                        case CATEGORY:

                            event.getGuild().getCategoryById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(member).deny(deny).queue();

                            break;

                        case STAGE:

                            event.getGuild().getStageChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(member).deny(deny).queue();

                            break;

                        case NEWS:

                            event.getGuild().getNewsChannelById(event.getChannel().getId()).getPermissionContainer().upsertPermissionOverride(member).deny(deny).queue();
                    ...
                    reply.setDescription("> Der Nutzer " + member.getAsMention() + " wurde erfolgreich vom Kanal entfernt!");
